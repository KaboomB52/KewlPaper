...
3. cd into `kewlpaper-server` for server changes, and `paper-api` for API changes.
**Only changes made in `kewlpaper-server/src/minecraft` have to deal with the patch system.**

`kewlpaper-server/src/minecraft` is not a git repositories in the traditional sense. Its
...
1. cd into `kewlpaper-server/src/minecraft/java` and run `git rebase -i base`;
...
1. Modify `kewlpaper-server/src/minecraft` with the appropriate changes;
